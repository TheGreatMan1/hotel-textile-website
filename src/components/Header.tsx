  ...
  return (
    <header className="sticky top-0 z-40 border-b border-stone-200/80 bg-[#fbf7ef]/92 shadow-[0_8px_28px_rgba(28,26,23,0.05)] backdrop-blur-xl transition-colors dark:border-stone-800 dark:bg-ink/90">
      <div className="container-shell flex min-h-14 items-center justify-between gap-3">
        ...
        >
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-brass/35 bg-champagne/20 text-brass dark:border-champagne/40 dark:bg-champagne/10 dark:text-champagne">
            <Sparkles aria-hidden size={19} strokeWidth={1.6} />
          </span>
          <span className="min-w-0">
            <span className="block truncate font-serif text-xl font-semibold leading-none text-charcoal transition group-hover:text-brass dark:text-ivory dark:group-hover:text-champagne">
              {brandMain}
            </span>
        ...
        >
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="relative text-[10px] font-bold uppercase tracking-[0.08em] text-stone-700 transition after:absolute after:-bottom-2 after:left-0 after:h-px after:w-0 after:bg-brass after:transition-all hover:text-brass hover:after:w-full dark:text-stone-300 dark:after:bg-champagne dark:hover:text-champagne"
            >
              {link.label}
            </a>
          ))}
